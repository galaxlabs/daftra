    ...
    def validate(self):
        ...
        if self.sales_invoice:
            ...
            if flt(self.amount) > flt(invoice.balance) and flt(invoice.balance) > 0:
                frappe.throw(_('Payment amount cannot exceed invoice balance'))
    ...
    def _resolve_payment_source(self):
        if self.bank_registration:
            ...
            if not self.reference:
                self.reference = bank.account_number[-4:]
        elif self.payment_method in {'Bank Transfer', 'Card', 'Online'} and not self.treasury:
        ...
        if self.payment_method in {'Bank Transfer', 'Card', 'Online'} and not self.bank_registration:
            frappe.throw(_('Select a Bank Registration for non-cash payments'))
